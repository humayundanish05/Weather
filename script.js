document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateCurrentWeather(data) {
        if (cityNameEl) cityNameEl.innerText = `${data.name}, ${data.sys.country}`;
        ...
        if (currentTempEl) currentTempEl.innerText = Math.round(data.main.temp);
        if (feelsLikeEl) feelsLikeEl.innerText = `${Math.round(data.main.feels_like)}°`; // New feature

        // Description & Icon
        const weather = data.weather[0];
        if (weatherDescEl) weatherDescEl.innerText = weather.description;
        if (weatherIconEl) {
        ...
        if (windSpeedEl) windSpeedEl.innerText = `${data.wind.speed} ${currentUnit === 'metric' ? 'km/h' : 'mph'}`;
        if (pressureEl) pressureEl.innerText = `${data.main.pressure} hPa`;
        if (visibilityEl) visibilityEl.innerText = `${(data.visibility / 1000).toFixed(1)} km`;

        // Sun
        if (sunriseEl) sunriseEl.innerText = formatTime(data.sys.sunrise, data.timezone);
        if (sunsetEl) sunsetEl.innerText = formatTime(data.sys.sunset, data.timezone);
